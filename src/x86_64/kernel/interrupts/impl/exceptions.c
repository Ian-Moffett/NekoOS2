...
void page_fault_ex(int_frame64_t* regs) {
    ...
    kputs_hex(regs->rflags & 0x1, &vga, 1);
    kputs_hex(regs->rflags & 0x2, &vga, 1);
    kputs_hex(regs->rflags & 0x4, &vga, 1);
    kputs_hex(regs->rflags & 0x8, &vga, 1);
    kputs_hex(regs->rflags & 0x10, &vga, 1);
